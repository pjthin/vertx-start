...
/**
 * Wrapper of {@link MongoClient} which use &lt;T> instead of {@link JsonObject}
 * when it's possible.
 * 
 * @author Pidji
 *
...
public interface MongoClientDataWrapper<T> {
	...
	/**
	 * Get {@link MongoClientDataWrapper} using reflection to provide a fromJson
	 * function and a toJson function (see
	...
	static <S> MongoClientDataWrapper<S> getInstance(MongoClient mongoClient, Class<S> klass) {
		return new MongoClientDataWrapperImpl<>(mongoClient, new ToJsonMethod<>(klass),
				new FromJsonConstructor<>(klass));
	}

	/**
	 * Get {@link MongoClientDataWrapper}.
	 * <p>
	 * The function provided will be called to map the POJO to
	 * {@link JsonObject} and vice-vers-ça.
	 * 
	 * @param mongoClient
	 * @param toJson
	 * @param fromJson
	...
	MongoClient update(String collection, JsonObject query, JsonObject update,
