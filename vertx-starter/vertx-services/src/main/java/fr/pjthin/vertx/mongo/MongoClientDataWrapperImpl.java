 ...
 *
 * @param <T>
 *            Class POJO with {@link JsonObject} constructor (like POJO annoted
 *            {@link DataObject})
 */
class MongoClientDataWrapperImpl<T> implements MongoClientDataWrapper<T> {
	...
	@Override
	public MongoClient replace(String collection, JsonObject query, T replace,
			Handler<AsyncResult<Void>> resultHandler) {
	...
	public MongoClient find(String collection, JsonObject query, Handler<AsyncResult<List<T>>> resultHandler) {
		return mongoClient.find(collection, query, h -> {
			if (h.succeeded()) {
				resultHandler
						.handle(Future.succeededFuture(h.result().stream().map(fromJson).collect(Collectors.toList())));
			}
			else {
	...
	public MongoClient findWithOptions(String collection, JsonObject query, FindOptions options,
		...
		return mongoClient.findWithOptions(collection, query, options, h -> {
			if (h.succeeded()) {
				resultHandler
						.handle(Future.succeededFuture(h.result().stream().map(fromJson).collect(Collectors.toList())));
			}
