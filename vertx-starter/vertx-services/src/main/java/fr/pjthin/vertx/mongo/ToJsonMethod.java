...
class ToJsonMethod<T> implements Function<T, JsonObject> {
	...
	public ToJsonMethod(Class<T> klass) {
		...
		this.method = method.orElseThrow(() -> new RuntimeException(
				String.format("Fail to find method type 'JsonObject methodName()' for %s", klass)));
	}
	...
	private JsonObject callMethod(T t) {
		try {
			...
			throw new RuntimeException(
					String.format("Fail to call method %s.%s for %s", method.getDeclaringClass(), method, t), e);
		}
	}
